...
/**
 * Fetch daily OHLCV bars for an explicit date window. Prefer this for long
 * backtests: Yahoo can silently downsample very long `range=max` chart calls,
 * while period-bounded daily requests keep actual daily bars.
 */
export async function fetchDailyHistoryWindow(
  ticker: string,
  startDate: string,
  endDate: string,
  timeoutMs: number = DEFAULT_TIMEOUT_MS,
  ...
  try {
    ...
    const end = Math.floor((new Date(`${endDate}T00:00:00Z`).getTime() + 24 * 3600 * 1000) / 1000);
    ...
    console.warn(`fetchDailyHistoryWindow(${ticker}) failed:`, e instanceof Error ? e.message : e);
    return null;
  }
}
